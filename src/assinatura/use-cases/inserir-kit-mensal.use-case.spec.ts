...
import { KitMensal, KitMensalInput, PlanoAssinatura } from '@assinatura/entities';
...
describe('InserirKitMensalUseCase', () => {
  ...
  let salvarKitMock: jest.MockedFunction<(kit: KitMensal) => Promise<KitMensal>>;
  ...
  beforeEach(() => {
    ...
    const kitMensalService = { salvarKit: salvarKitMock } as unknown as KitMensalService;

    useCase = new InserirKitMensalUseCase(kitMensalService, planoService);
  });
  ...
  it('deve lançar NotFoundException quando o plano não existe', async () => {
    ...
    await expect(
      useCase.execute({ idPlano: 99, mesReferencia: 4, anoReferencia: 2026, itens: [] }),
